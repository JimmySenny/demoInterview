...
#    def maxSubArray(self, nums: List[int]) -> int:
    ...
    def maxSubArray(self, nums):
        ...
        return m;
    #滑动窗口优化,用sum_sub变量保存num[i]开始至nums[j]的子数组之和
...
#将子问题定义为求以i为终止下标的子数组之和的最大值，这样最终可以通过比较以0为下标的子数组最大值、以1为下标的子数组最大值......以n-1为下标的子数组最大值获得最终解。我们用数组dp来记录子问题的解。递推基为dp[0]=array[0]。递推关系相比下来就清晰明了多了，当我们对dp[i-1]和array[i]分正负讨论。若dp[i-1]<0, array[i]>0，dp[i]=array[i]，若dp[i-1]<0, array[i]<0, dp[i]=array[i]，若dp[i-1]>0, array[i]<0, dp[i]=dp[i-1]+array[i], 若dp[i-1]>0, array[i]>0, dp[i]=dp[i-1]+array[i]。最后, 原始问题的解即max(dp)。
    def maxSubArrayDP(self, nums):
        ...
        i = 1;
        while i < l:
            print(i,max(dp[i-1]+nums[i],nums[i]))
            dp[i] = max(dp[i-1] + nums[i],nums[i]) if i else nums[0]
            i += 1
...
#kadane算法是在动态规划解的基础上进一步优化。它使用一根指针保存以i为结尾的子数组最大值之和，使用另一根指针保存迄今为止的子数组最大值之和。算法时间复杂度为O（n）,空间复杂度为O(1))
    def maxSubArraykadane(self, nums):
        l = len(nums)
        max_end = max_sub = nums[0]
        i = 1
        while i < l:
            max_end = max(max_end + nums[i], nums[i])
            max_sub = max(max_end, max_sub)
            i += 1
        return max_sub;
